# Novel Architecture for Solidity Smart Contract Vulnerability Detection using Diffusion Models

**SolidityDiffusion** - a novel architecture that adapts the diffusion model paradigm for smart contract vulnerability detection.

## 1. Why Current Approaches Fall Short for Solidity
...
## 2. Proposed Novel Architecture: **SolidityDiffusion**
...
## 6. Why This Beats Current Tools
...
class FunctionLevelDiffusion(nn.Module):
    ...
    def __init__(self, input_dim=512, hidden_dim=768, num_heads=8, num_layers=8):
        super().__init__()
        # Similar structure to ContractLevelDiffusion but different capacity
        self.time_embed = nn.Sequential(
            nn.Linear(1, 256),
            nn.SiLU(),
            nn.Linear(256, hidden_dim)
...
This is a **2-3 top-tier conference paper** (USENIX Security, IEEE S&P, CCS):
- Paper 1: Economic Flow Graphs + Basic Diffusion
- Paper 2: Hierarchical Architecture + Vulnerability-Specific Denoisers
- Paper 3: Attack Scenario Generation + Automated Repair
